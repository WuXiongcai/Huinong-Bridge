<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入民俗文化名称" style="width: 200px" prefix-icon="el-icon-search" v-model="name"></el-input>
    ...
    <div class="operation">
      <el-button type="primary" icon="el-icon-plus" @click="handleAdd">新增民俗文化</el-button>
    ...
    <div class="table">
      <el-table :data="tableData" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="title" label="民俗名称"></el-table-column>
        <el-table-column prop="category" label="分类">
          <template v-slot="scope">
            <el-tag :type="scope.row.category === '节日习俗' ? 'success' : 
                          scope.row.category === '民间艺术' ? 'warning' : 'info'">
              {{ scope.row.category }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <!-- 新增/编辑弹窗 -->
    <el-dialog 
        :title="form.id ? '编辑民俗文化' : '新增民俗文化'" 
        :visible.sync="dialogVisible" 
        width="65%" 
        :close-on-click-modal="false"
        :destroy-on-close="true"
        custom-class="folk-dialog">
        <el-form :model="form" :rules="rules" ref="formRef" label-width="100px" class="folk-form">
            <el-row :gutter="20">
                <el-col :span="12">
                    <el-form-item label="民俗名称" prop="title">
                        <el-input v-model="form.title" placeholder="请输入民俗文化名称"></el-input>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="分类" prop="category">
                        <el-select v-model="form.category" placeholder="请选择分类" style="width: 100%">
                            <el-option label="节日习俗" value="节日习俗">
                                <i class="el-icon-date"></i>
                                <span style="margin-left: 8px">节日习俗</span>
                            </el-option>
                            <el-option label="民间艺术" value="民间艺术">
                                <i class="el-icon-picture"></i>
                                <span style="margin-left: 8px">民间艺术</span>
                            </el-option>
                            <el-option label="传统礼仪" value="传统礼仪">
                                <i class="el-icon-collection-tag"></i>
                                <span style="margin-left: 8px">传统礼仪</span>
                            </el-option>
                        </el-select>
                    </el-form-item>
                </el-col>
        ...
        <div class="view-content">
            <div class="view-header">
                <div class="view-category">
                    <el-tag :type="viewCategory === '节日习俗' ? 'success' : 
                                   viewCategory === '民间艺术' ? 'warning' : 'info'">
                        {{ viewCategory }}
                    </el-tag>
                </div>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        title: [{ required: true, message: '请输入民俗名称', trigger: 'blur' }],
  ...
  methods: {
    ...
    save() {
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          ...
          this.$request({
            url: this.form.id ? '/folk/update' : '/folk/add',
    ...
    del(id) {
      this.$confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/folk/delete/' + id).then(res => {
          if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/folk/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {
            this.$message.success('操作成功')
            this.load(1)
          } else {
...
.folk-dialog /deep/ .el-dialog__header {
    padding: 20px 25px;
    border-bottom: 1px solid #eee;
}

.folk-dialog /deep/ .el-dialog__body {
    padding: 25px;
}
